...
export default function Fastest(props){
    ...
    const fetchData = async () => {
        let url = ""
        url = props.type === "gov"
        ? `/init/employees/fast/top-8/gov/${props.govId !== '' ? props.govId : user?.governorateId}`
        : `/init/employees/fast/top-8/city/${props.cityId !== '' ? props.cityId : user?.cityId}`;
        const res = await Axios.get(url)
        return res.data
    }

    const {data , isLoading, error} = useQuery({
        queryKey: ['fastest' , props.govId , props.cityId],
        queryFn: fetchData,
        staleTime: 1000 * 60,
    })
    ...
    if (error) {
        return (
            <div className="bg-white dark:bg-[#191A1A] rounded-md min-h-[476.5px] pt-4 flex flex-col">
                <h1 className="w-fit text-2xl mx-2 relative before:absolute before:h-[1px] before:w-[calc(100%)] before:bg-slate-300 before:dark:bg-[#363D3E] before:right-0 before:bottom-[-15px] after:absolute after:w-[40%] after:h-[2px] after:bg-[#725DFE] dark:text-white after:bottom-[-15px] after:right-0">اسرع الموظفيين</h1>
    ...
    // Convert object to array of employees
    const employees = data ? Object.entries(data).map(([name, time]) => ({
        name,
        time
    })) : [];
    
    const showData = employees.length > 0 ? employees.map((el, index) => (
        <div key={index} className={`flex justify-between items-center py-[0.81rem] px-2 ${index % 2 === 0 ? "": "dark:bg-[#1D1F20] bg-[#F4F7FA]"}`}>
            <h2 className="text-sm">{el.name || 'غير محدد'}</h2>
            <p className="flex items-center text-sm gap-1">
                <span>{el.time !== undefined ? Math.floor(el.time * 10)/10 : 0}h</span>
